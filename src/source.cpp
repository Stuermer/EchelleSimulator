...
void Source::scale_spectral_density() {
    ...
    double area = 0.0;  // signed area
    for (int i = 0; i < n; i++) {
        area += this->get_spectral_density(a + (i + 0.5) * step) * (inten_pho) * (a + (i + 0.5) * step) *
                (step); // sum up each small rectangle
    }
...
double Blackbody::planck(const double &T, const double &wavelength) {
    ...
    double intensity =
            (s_val) * (c) * a / (pow(wavelength, 5) * (exp(b) - 1.0)); // (J / s ) / m^3 -> (uJ / s) / ( m^2 * um )
    return intensity;
}
